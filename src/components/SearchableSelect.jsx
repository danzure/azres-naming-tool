                ...
                {isOpen && (
                    <div className={`absolute top-[100%] left-0 right-0 z-[100] shadow-lg border rounded overflow-hidden mt-1 ${isDarkMode ? 'bg-[#252423] border-[#484644]' : 'bg-white border-[#edebe9]'}`}>
                        <div className="p-2 border-b border-opacity-10 border-current">
                            <input autoFocus type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={placeholder} className={`w-full px-2 py-1.5 text-[13px] border-b outline-none bg-transparent ${isDarkMode ? 'text-white border-[#484644] placeholder:text-[#605e5c]' : 'text-[#201f1e] border-[#edebe9] placeholder:text-[#a19f9d]'}`} />
                        </div>
                        <div className="max-h-[300px] overflow-y-auto">
                            {filteredItems.map((item, idx) => {
                                ...
                                return (
                                    <div key={item.value} onClick={() => { onChange(item.value); setIsOpen(false); setSearch(''); }} className={`flex items-center justify-between px-3 py-2 text-[13px] cursor-pointer transition-colors ${value === item.value ? (isDarkMode ? 'bg-[#292827] text-white font-semibold' : 'bg-[#f3f2f1] text-[#201f1e] font-semibold') : (isDarkMode ? 'text-[#ffffff] hover:bg-[#323130]' : 'text-[#201f1e] hover:bg-[#edebe9]')}`}>
